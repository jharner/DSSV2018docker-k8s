...
These technologies form the basis for solving the data analysis issues mentioned above. Docker Swarm is also an orchestation option, but not as scalable or feature complete as Kubernetes. At the other extreme, Mesos is for urge containerization deployments.
...
* immutable infrasructure: update an image with a newer image in a single operation, i.e., no incremantal updates (except in emergencies);  
* declarative configuration: describe the desired state of your application.

In mutable infrastructures changes are applied incremetally, e.g., `apt-get` in Linux or `install.packages` in R. This should only be done as a temporary fix. An imperative declaration defines the state as a series of actions rather than a declaration of the desired state as is done with Kubernetes.

Kubernetes make sure the actual state or your environment is aligned with the desired state. Not only that, Kubernetes is an online, self-healing system, i.e., it continuously takes action to ensure the current state matches the desired state.

## Kubernetes Architecture

The machines in a Kubernetes cluster are classified as:

* a master server (or several servers): the primary point of contact with the cluster. It exposes the API, aligns the actual state to the desired state, schedules work, orchestrates communication, etc.  
...
The basic Kubernetes unit is the pod. Containers are assigned to pods---not hosts. Pods should contain a single application or related applications, e.g., one that need to share the same filesystem.

## Container Platform

A container platform provides a complete solution, e.g., the components needed for teaching a data science program---perhaps a subset of the above streaming architecture.
...
Kubernetes is now available on AWS, Azure, and of course Google Cloud. However, these cloud providers do not have predictable price models and users/developers can incur large unexpected costs. Digital Ocean gives the user far more control over these costs, i.e., limits can be set and the user is only charged for what is actually used.

That said---it would be easy to deplay RSpark to any cloud provider using Kubernetes.

But why is Kubernetes necessary? Primarily scalability and reliability. For example, the Spark cluster component can be scaled based on the user's needs.
